...
IMAGEPATH = "F:/t8_28_2/t8_27_wanshang/t8_26/t8/project_name/static/js/assets/img/lingxiu/"
IMAGROOTPATH = "F:/t8_28_2/t8_27_wanshang/t8_26/t8/project_name/static/js/assets/img/lingxiu/"
    ...
    def get_from_mongodb(self, table, topic):
        ...
        for i in table.find():
            if str(i['topic']) == str(topic):
                return i
    def get_from_questionall(self,table,topic):
        list1=[]
        count_all=0
        count_top=[]
        table=self.db[table]
        for i in table.find({"topic":topic}):
            list1.append(i["question_list"])
        #print(list1[-1])
        for j,ii in zip(range(len(list1[-1])),list1[-1]):
            if j<10:
                count_top.append(ii["answer_count"])
            count_all+=ii["answer_count"]
        return count_all,count_top
    def niji(self,set):
        # 计算回答的基尼系数、
        niji=set[1]
        list_qnum = []
        ...
        paint.zhitu(list_p)
        print (gini_coef(list_qnum))
        return gini_coef(list_qnum)
...
def search_process(client, topic="龙珠改魔人布欧"):
    ...
    z_top_question_dict = {
        "topic": topic,
        "question_list": z_top_question_list
    }
    for count, result in zip(range(4), client.search(topic, search_type="GENERAL")):
        if not isinstance(result, SearchResultSection):
            ...
            if isinstance(r.obj, Answer):
                ...
                z_top_question_list.sort(key=lambda question: question["answer_count"], reverse=True)

        print('-' * 20)
    z_top_question_dict["question_list"] = z_top_question_dict["question_list"][:8]
    # 存取按答案数排序后前八个问题
    ZhihuStore_object.store_into_mongodb(table=TOPQUESTION, dict_info=z_top_question_dict)
    for z_question_dict in z_top_question_dict["question_list"]:
        z_question = client.question(int(z_question_dict["question_id"]))
    ...
    ZhihuStore_object.store_into_mongodb(table=ALLQUESTION, dict_info=z_all_question_dict)
...
#     for imgurl in img:
#         urllib.urlretrieve(imgurl, IMAGEPATH+'%s.jpg' % x)
#     return True
...
def zht(url,index):
    ...
    try:
        ...
        for imgurl in img:
            print(imgurl)
            urllib.urlretrieve(imgurl, IMAGEPATH + '10'+'%s.jpg' % index)
    except Exception,e:
        #print 'time out'+a+b+c
        pass
...
#         with open(IMAGEPATH+str(int(index)+1)+'.jpg', 'wb') as f:
#             f.write(t_content)
#
#     except:
#         return False
...
class ZhihuMongo(object):
    ...
    def get_top_people_from_mongodb(self):
        ...
        for index, value in enumerate(top_list):
            url = 'https://www.zhihu.com/question/'+str(value['this_answer_question_id'])+'/answer/' + str(value['this_answer_id'])
            with open('F:/hhh.txt', 'a') as f:
                f.write(url)
            time.sleep(0.5)
    ...
    def get_ciyun1(self, type1):
        ...
        my_wordcloud.to_file(IMAGROOTPATH+"z_" + type1 + ".jpg")
#
...
if __name__ == '__main__':
    zht(url="https://www.zhihu.com/question/51499017/answer/126360417", index=9999999)
    ...
    # a=ZhihuStore()
    # set1=a.get_from_questionall(ALLQUESTION,'211')
    # a.niji(set1)
