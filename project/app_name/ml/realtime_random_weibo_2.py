...
import random
import fenci
from realtime_user_fans_follower import Realtime_threaten_user_relationship
reload(sys)
sys.setdefaultencoding('utf-8')

REALTIMEWEIBO = 'realtime719'
REALTIMEWEIBOT = 'realtime719t'
TEMPRALATIONSHIP = 'Relation728'
...
class RealtimeRandomWeibo(object):
    ...
    def store_to_mongodb(self):
        ...
        self.get_threaten_relationship_network()

    def get_threaten_relationship_network(self):
        for weibo in self.weibo_list_threat:
            import time
            time.sleep(0.1)
            print(888888888888888888888888888888888888888)
            print(888888888888888888888888888888888888888)
            author_uid = weibo["author_uid"]
            Realtime_threaten_user_relationship(author_uid, weibo)
    ...
    def get_realtime_weibo_from_mongodb_for_test(self):
        weibo_table = self.db[REALTIMEWEIBO]
        now_weibo_all = []
        for i in weibo_table.find():
            now_weibo_all.append(i)
        return  now_weibo_all[-15:]
    ...
    def get_temp_threaten_relationship_from_mongodb(self):
        ...
        for_returned_weibo_list = []
        for i in now_weibo_threaten:
            if i["uid"] not in set_:
                set_.add(i["uid"])
                for_returned_weibo_list.append(i)

        # for i in range(0, now_weibo_threaten.__len__())[::-1]:
        #     for returned_weibo in for_returned_weibo_list:
        #         if str(returned_weibo["uid"].strip())==str(now_weibo_threaten[i]["uid"].strip()):
        #             print(9999999999999999999999)
        #             continue
        #     for_returned_weibo_list.append(now_weibo_threaten[i])
        #     if len(for_returned_weibo_list)>=7:
        #         break

        return for_returned_weibo_list[-7:]
