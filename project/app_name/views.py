...
def monitor(request):       #检测的地图
    ...
    realtime_object_1.get_random_weibo()
    weibo_list = realtime_object_1.get_realtime_weibo_from_mongodb_for_test()
    for weibo in weibo_list:
        if weibo['is_repost']:
            List1.append(
                [
...
def weibo_data(request):    #微博数据页面
    ...
    id1=weibo_threaten_2[0]['author_uid']
    #敏感微博的 id author location time content  共5个
    author1=weibo_threaten_2[0]['author_name']
    location1=weibo_threaten_2[0]['location']
    time1=weibo_threaten_2[0]['time']
    content1=weibo_threaten_2[0]['text']
    id2=weibo_threaten_2[1]['author_uid']
    author2=weibo_threaten_2[1]['author_name']
    location2=weibo_threaten_2[1]['location']
    time2=weibo_threaten_2[1]['time']
    content2=weibo_threaten_2[1]['text']
    id3=weibo_threaten_2[2]['author_uid']
    author3=weibo_threaten_2[2]['author_name']
    location3=weibo_threaten_2[2]['location']
    time3=weibo_threaten_2[2]['time']
    content3=weibo_threaten_2[2]['text']
    id4=weibo_threaten_2[3]['author_uid']
    author4=weibo_threaten_2[3]['author_name']
    location4=weibo_threaten_2[3]['location']
    time4=weibo_threaten_2[3]['time']
    content4=weibo_threaten_2[3]['text']
    id5=weibo_threaten_2[4]['author_uid']
    author5=weibo_threaten_2[4]['author_name']
    location5=weibo_threaten_2[4]['location']
    time5=weibo_threaten_2[4]['time']
    content5=weibo_threaten_2[4]['text']
    # List1-List4 是关系图的参数
    List1 = []
    List2 = []
    List3 = []
    ...
    for weibo in weibo_threaten_2:
        ...
        List1.append(
            {
                "name": weibo['author_name'],
                "keyword": {},
                "base": weibo['author_name']
            }
        )
        List2.append(
            {
    ...
    for person in weibo_threaten_2:
        List4.append(person['author_name'])
    ...
    list33_max_value = max(list33)
    for index, id_num in enumerate(list33):
        if id_num == list33_max_value:
            list33_max_index = provice_id_list[index]
            break

    list33_min_value = min(list33)
    for index, id_num in enumerate(list33):
        if id_num == list33_min_value:
            list33_min_index = provice_id_list[index]
            break
    # List11-List44 是微博柱状图
    List11 =  ['山西','山东','河南','河北','陕西', '江苏','广东','新疆','浙江','台湾','北京','上海', '辽宁']   #横坐标 为地区
    List22 = list22            #红色 微博量的数值
    List33 = list33              #灰色 敏感量的数值
    List44 = [  {'name' : '最高', 'value' : list33_max_value, 'xAxis': list33_max_index, 'yAxis': list33_max_value},                    #柱状图的最高最低的 横坐标 纵坐标
                    {'name ': '最低', 'value' : list33_min_value, 'xAxis': list33_min_index, 'yAxis': list33_min_value}]
...
def zhihu_data(request):
    ...
    p5 =  0.00447750148017
    answer_words5 = '"本科" "清北" "硕士" "211" "高晓松" "清华" "学历" "出身" "一个" "博士" "985" "公平"'
    id6 = 6
    tittle6 = "985、211 高校毕业的电气硕士，去国家电网合适吗？"
    p6 = 	0.0036464462751
    answer_words6 = '"国网" "电网" "甘心" "同学" "自己" "行业" "科研" "国企" "工作" "进入" "选择" "毕业"'
    id7 = 7
    tittle7 = "如何从非 211 的一本大学考研到名牌大学？"
    p7 = 0.00307564861816
    answer_words7 = '"考研" "自己" "复习" "专业课" "学校" "专业" "英语" "真题" "数学" "本科" "复试" "很多"'
    id8 = 8
...
def search(request, a):
    ...
    if flag != 1:
        ...
        if len(address1)<20:
            address1 += " "*20
            address1 = " "*20 + address1
        if len(address2)<20:
            address2 += " "*20
            address2 = " "*20 + address2
        if len(address3)<20:
            address3 += " "*20
            address3 = " "*20 + address3
        if len(address4)<20:
            address4 += " "*20
            address4 = " "*20 + address4
        if len(address5)<20:
            address5 += " "*20
            address5 = " "*20 + address5
        if len(address6)<20:
            address6 += " "*20
            address6 = " "*20 + address6
        if len(address7)<20:
            address7 += " "*20
            address7 = " "*20 + address7
        if len(address8)<20:
            address8 += " "*20
            address8 = " "*20 + address8
            ...
            List5.append(
                {
                    ...
                    "user_location": person['this_answer_question_id'],
    ...
    zhihustore_object_1 = ZhihuStore()
    set1 = zhihustore_object_1.get_from_questionall(ALLQUESTION, str(zhihumongo_object_1.topic))
    zhihustore_object_1.niji(set1)
